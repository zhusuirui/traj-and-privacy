...
class SAM_LSTMCell(RNNCellBase):
    def __init__(self, input_size, hidden_size, grid_size, bias=True, incell = True):
        ...
        self.atten = Attention(hidden_size).to(device)
        self.c_d = None
        self.sg = None
        self.reset_parameters()
    ...
    def spatial_lstm_cell(self, input_a, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
        # self.spatial_embedding = torch.ones(self.spatial_embedding.size()).to(device)
        input = input_a[:,:-2]
        grid_input = input_a[:,-2:].type(torch.LongTensor).to(device) + config.spatial_width

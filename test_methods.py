...
def test_comput_embeddings(self, spatial_net, test_batch = 1025):
    if config.recurrent_unit=='GRU' or config.recurrent_unit=='SimpleRNN':
        hidden = autograd.Variable(torch.zeros(test_batch, self.target_size), requires_grad=False).to(device)
    else:
        hidden = (autograd.Variable(torch.zeros(test_batch, self.target_size), requires_grad=False).to(device),
                  autograd.Variable(torch.zeros(test_batch, self.target_size), requires_grad=False).to(device))
    embeddings_list = []
    j = 0
    s = time.time()
...
def test_comput_embeddings_init(self, spatial_net, test_batch = 1025):
    if config.recurrent_unit=='GRU' or config.recurrent_unit=='SimpleRNN':
        hidden = autograd.Variable(torch.zeros(test_batch, self.target_size), requires_grad=False).to(device)
    else:
        hidden = (autograd.Variable(torch.zeros(test_batch, self.target_size), requires_grad=False).to(device),
    ...
    j = 0
    s = time.time()
    while j < self.padded_trajs_init.shape[0]:
        for i in range(self.batch_size):
            out = spatial_net.rnn([autograd.Variable(torch.Tensor(self.padded_trajs_init[j:j+test_batch]),
                                                     requires_grad=False).to(device),
                                   self.trajs_length[j:j+test_batch]],hidden)
            # embeddings = out.data.cpu().numpy()
            embeddings = out.data
...
def test_model_new(self, traj_embeddings,traj_embeddings_init ,test_range, print_batch=10, similarity = False, r10in50 = False):
    ...
    for i in test_range:
        if similarity:
            ...
            # test_distance_init = [(j, float(np.exp(-np.sum(np.square(traj_embeddings[i] - e)))))
            #                  for j, e in enumerate(traj_embeddings_init)]
            test_distance=[(j, float(np.exp(-np.sum(np.square(traj_embeddings_init[i] - e)))))
                             for j, e in enumerate(traj_embeddings)]
            ...
            s_learned_distance = sorted(learned_distance, key=lambda a: a[1])
        top1_recall=[]
        if i in [j[0] for j in s_test_distance[:1]]:
            top1_recall.append(s_test_distance[0][0])
        top5_recall=[]
        if i in [j[0] for j in s_test_distance[:5]]:
            top5_recall.append(s_test_distance[0][0])
        top10_recall=[]
        if i in [j[0] for j in s_test_distance[:10]]:
            top10_recall.append(s_test_distance[0][0])
        top50_recall=[]
        if i in [j[0] for j in s_test_distance[:50]]:
            top50_recall.append(s_test_distance[0][0])
    ...
    print ('Search Top 5 recall {}'.format(float(top_5_count) / (test_traj_num * 1)))
    print ('Search Top 50 recall {}'.format(float(top_50_count) / (test_traj_num * 1)))
    print ('Search Top 10 recall {}'.format(float(top_10_count) / (test_traj_num * 1)))

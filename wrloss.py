...
class WeightMSELoss(Module):
    def __init__(self, batch_size, sampling_num):
        ...
        self.weight = Parameter(torch.Tensor(self.weight.astype(float)).to(device), requires_grad = False)
...
class WeightedRankingLoss(Module):
    ...
    def forward(self, p_input, p_target, n_input, n_target):
        trajs_mse_loss = self.positive_loss(p_input, autograd.Variable(p_target).to(device), False)

        negative_mse_loss = self.negative_loss(n_input, autograd.Variable(n_target).to(device), True)

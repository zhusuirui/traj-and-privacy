...
def trajectory_feature_generation(path ='./data/toy_trajs',
    ...
    with open(path, 'rb') as f:
        trajs = cPickle.load(f, encoding='bytes')
    ...
    with open('./features/{}_traj_index'.format(fname), 'wb') as outfile:
        cPickle.dump(traj_index, outfile)
    ...
    print (trajs[0])
    with open('./features/{}_traj_coord'.format(fname), 'wb') as outfile:
        cPickle.dump((trajs,[],max_len), outfile)
    ...
    with open('./features/{}_traj_grid'.format(fname), 'wb') as outfile:
        cPickle.dump((all_trajs_grids_xy,[],max_len), outfile)

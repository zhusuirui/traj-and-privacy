...
class NeuTrajTrainer(object):
    ...
    def data_prepare(self, griddatapath = config.gridxypath,
        ...
        with open(griddatapath, 'rb') as f:
            traj_grids, useful_grids, max_len = cPickle.load(f, encoding='bytes')
        ...
        t_max=85640
        s_max=0.7434750672147674

        with open(coordatapath, 'rb') as f:
            traj_grids, useful_grids, max_len = cPickle.load(f, encoding='bytes')

        time_start=time.time()
        delta = 10e-5
        epsilon = 0.005
        sigma = np.sqrt(2 * np.log(1.25 / delta)) * (s_max/3000) / epsilon
        delta_time = 10e-5
        epsilon_time = 0.0005
        sigma_time = np.sqrt(2 * np.log(1.25 / delta_time)) * (t_max/3000) / epsilon_time
        # time_end = time.time()
        # aaaaa=time_end-time_start
        traj_grids_noise1=[]
        a=np.random.normal(loc=0, scale=sigma)
        for traj1 in traj_grids:
            temp1=[]
            for r1 in traj1:
                if np.random.random()>-1:
                    # r2=[r1[0]+np.random.normal(loc=0, scale=sigma),
                    #     r1[1]+np.random.normal(loc=0, scale=sigma),
                    #     r1[2]+np.random.normal(loc=0, scale=sigma_time)]
                    r2=[r1[0]+np.random.normal(loc=0, scale=sigma),
                        r1[1]+np.random.normal(loc=0, scale=sigma),
                        r1[2]]
                    temp1.append(r2)
                    ...
                    temp1.append(r1)
            traj_grids_noise1.append(temp1)
        # time_end = time.time()
        # aaaaa=time_end-time_start
        x_noise, y_noise,z_noise = [], [],[]
        for traj_noise in traj_grids_noise1:
            for r_noise in traj_noise:
                x_noise.append(r_noise[0])
                y_noise.append(r_noise[1])
                z_noise.append(r_noise[2])
        meanx_noise, meany_noise, stdx_noise, stdy_noise = \
            np.mean(x_noise), np.mean(y_noise), np.std(x_noise), np.std(y_noise)
        traj_grids_noise=[]
        for t_noise in traj_grids_noise1:
            temp_noise=[]
            for r_noise in t_noise:
                temp_noise.append([(r_noise[0] - meanx_noise) / stdx_noise,
                                         (r_noise[1] - meany_noise) / stdy_noise,
                                         (math.sin(2 * math.pi * r_noise[2] /86400) + 1) / 2,
                                         (math.cos(2 * math.pi * r_noise[2] /86400) + 1) / 2
                                          ])
            traj_grids_noise.append(temp_noise)
        ...
        traj_grids = [[[(r[0] - meanx) / stdx, (r[1] - meany) / stdy,
                        (math.sin(2 * math.pi * r[2] / 86400) + 1) / 2,
                        (math.cos(2 * math.pi * r[2] / 86400) + 1) / 2
                        ] for r in t] for t in traj_grids]
        coor_trajs_noise = traj_grids_noise[:dataset_length]
        ...
        coor_train_seqs_noise, coor_test_seqs_noise = \
            coor_trajs_noise[:train_size], coor_trajs_noise[train_size:]

        self.grid_trajs = grid_trajs
        self.grid_train_seqs = grid_train_seqs
        self.coor_trajs = coor_trajs
        self.coor_trajs_noise=coor_trajs_noise
        self.coor_train_seqs = coor_train_seqs_noise

        pad_trjs = []
        for i, t in enumerate(grid_trajs):
            traj = []
            for j, p in enumerate(t):
                traj.append([coor_trajs_noise[i][j][0], coor_trajs_noise[i][j][1],
                             coor_trajs_noise[i][j][2], coor_trajs_noise[i][j][3],
                             p[0], p[1]])
            pad_trjs.append(traj)

        pad_trjs_init = []
        for i_init, t_init in enumerate(grid_trajs):
            traj_init = []
            for j_init, p_init in enumerate(t_init):
                traj_init.append([coor_trajs[i_init][j_init][0],
                                  coor_trajs[i_init][j_init][1],
                                  coor_trajs[i_init][j_init][2],
                                  coor_trajs[i_init][j_init][3],
                                  p_init[0], p_init[1]])
            pad_trjs_init.append(traj_init)
        ...
        self.padded_trajs_init = np.array(pad_sequence(pad_trjs_init, maxlen= max_len))
        # distance = cPickle.load(open(distancepath,'r'))
        with open(distancepath, 'rb') as f:
            distance = cPickle.load(f, encoding='bytes')
        ...
        while j< len(train_seqs):
            ...
            for i in range(self.batch_size):
                ...
                for traj_index in sampling_index_list:
                    ...
                    distance.append(float(train_distance[j + i][traj_index]))
                for traj_index in negative_sampling_index_list:
                    ...
                    negative_distance.append(float(train_distance[j + i][traj_index]))
                    if traj_index not in batch_trajs_keys:
                        batch_trajs_keys[j + i] = 0
                        batch_trajs_input.append(train_seqs[traj_index])
                        batch_trajs_len.append(self.trajs_length[traj_index])
    ...
    def neutraj_train(self, print_batch = 10, print_test = 100, save_model = True, load_model = None,
        ...
        spatial_net = NeuTraj_Network(130, self.target_size, self.grid_size, self.batch_size, self.sampling_num,
                                      stard_LSTM= stard_LSTM, incell= in_cell_update)
        ...
        for epoch in range(config.epochs):
            ...
            embeddings = tm.test_comput_embeddings(self, spatial_net, test_batch= config.em_batch)
            embeddings_init= tm.test_comput_embeddings_init\
            ...
            acc1 = tm.test_model_new(self,embeddings,embeddings_init , test_range=range(int(len(self.padded_trajs)*1/3),
                                                                   int(len(self.padded_trajs)*1/3)+config.test_num),
                                 similarity=True, print_batch=print_test)
